...
@interface HttpRequestSDK()
@property(nonatomic,assign)BOOL requestQueueFail;
@end
...
{
    //检查网络状态
    if([[RequestEngine Instance].reachabilityManager networkReachabilityStatus]>0)
    {
        //打印初始日志
        ...
        [[RequestEngine Instance]requestWithURL:url parameters:parameters successBlock:^(id json) {
            ...
            NSString *responseStr = [[NSString alloc] initWithData:json
         ...
         queueFailBlock:(void(^)(void))queueFailBlock;
{
    ...
    {
        //发送显示等待提示框信息
        if(self.showWait&&queueType==NormalQueue)
        {
            [[NSNotificationCenter defaultCenter]postNotificationName:@"ShowWait" object:nil];
        }
        ...
        [[RequestEngine Instance]requestWithQueue:operations
                             ...
                             queueCompletionBlock:^(NSArray *operations) {
                                 if(self.requestQueueFail)
                                 {
                                     self.requestQueueFail=NO;
                                     NSLog(@"n队列中存在错误请求\n");
                                    
                                     //发送取消等待提示框信息
                                     if(queueType==NormalQueue)
                                     {
